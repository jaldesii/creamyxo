...
io.on('connection', (socket) => {
  ...
  if (bannedIPs.has(ip)) { 
    socket.emit('banned', { message: 'Banned from Cremyxo.' }); 
    socket.disconnect(); 
    return; 
  }
  ...
  activeAnnouncements.forEach((announcement, id) => {
    ...
    if (announcement.duration === 0 || remaining > 0) { 
      socket.emit('announcement', { id: announcement.id, message: announcement.message, type: announcement.type, from: announcement.from || 'Cremyxo Admin', time: announcement.time, duration: announcement.duration, remaining: announcement.duration > 0 ? remaining : null }); 
    }
  });
  ...
  socket.on('admin-kick', (data) => { 
    const ts = io.sockets.sockets.get(data.socketId); 
    if (ts) { 
      ts.emit('kicked', { message: data.reason || 'Kicked.' }); 
      ts.disconnect(); 
    } 
  });
  
  socket.on('admin-ban', (data) => { 
    const u = allUsers.get(data.socketId); 
    if (u) { 
      bannedIPs.add(u.ip); 
      saveJSON(FILES.bannedIPs, [...bannedIPs]); 
      const ts = io.sockets.sockets.get(data.socketId); 
      if (ts) { 
        ts.emit('banned', { message: 'Banned.' }); 
        ts.disconnect(); 
      } 
    } 
  });
  ...
  socket.on('admin-mute', (data) => { 
    mutedUsers.add(data.socketId); 
    saveJSON(FILES.mutedUsers, [...mutedUsers]); 
    const ts = io.sockets.sockets.get(data.socketId); 
    if (ts) ts.emit('muted', { message: 'Muted.' }); 
    broadcastAdminUpdate(); 
  });
  
  socket.on('admin-unmute', (data) => { 
    mutedUsers.delete(data.socketId); 
    saveJSON(FILES.mutedUsers, [...mutedUsers]); 
    broadcastAdminUpdate(); 
  });
  ...
  socket.on('admin-close-room', (data) => { 
    if (activeRooms.has(data.roomId)) { 
      io.to(data.roomId).emit('system-message', { text: 'Room closed by admin.' }); 
      io.to(data.roomId).emit('partner-left'); 
      const r = activeRooms.get(data.roomId); 
      r.users.forEach(u => { 
        const uu = allUsers.get(u.id); 
        if (uu) uu.room = null; 
      }); 
  ...
  socket.on('admin-announce', (data) => {
    ...
    activeAnnouncements.set(announcementId, announcement); 
    saveJSON(FILES.announcements, [...activeAnnouncements.values()]);
    ...
    if (data.duration && data.duration > 0) { 
      setTimeout(() => { 
        ...
        saveJSON(FILES.announcements, [...activeAnnouncements.values()]); 
        io.emit('announcement-dismiss', { id: announcementId }); 
      }, data.duration * 1000); 
    }
  ...
  socket.on('find-partner', (userData) => {
    ...
    for (const [rid, r] of activeRooms.entries()) { 
    ...
    if (waitingUsers.length > 0) {
      ...
      if (pi === -1) { 
        waitingUsers.push(userObj); 
        socket.emit('waiting'); 
        return; 
      }
      const partner = waitingUsers.splice(pi, 1)[0]; 
      ...
      const pu = allUsers.get(partner.id), cu = allUsers.get(socket.id); 
      if (pu) pu.room = roomId; 
      if (cu) cu.room = roomId;
      ...
      io.to(roomId).emit('partner-status', { online: true }); 
      broadcastAdminUpdate();
    } else { 
      waitingUsers.push(userObj); 
      socket.emit('waiting'); 
  ...
  socket.on('send-message', (data) => {
    if (mutedUsers.has(socket.id)) { 
      socket.emit('system-message', { text: 'You are muted.' }); 
      return; 
    }
    ...
    if (data.message.type === 'voice') { 
      totalVoiceMessages++; 
      totalVoiceDuration += data.message.duration || 0; 
    }
  ...
  socket.on('report-user', (data) => { 
    reportedUsers.push({ reportedBy: data.reportedBy, reportedUser: data.reportedUser, reason: data.reason, time: new Date().toLocaleTimeString(), roomId: data.roomId }); 
    if (reportedUsers.length > 100) reportedUsers.shift(); 
    saveJSON(FILES.reportedUsers, reportedUsers); 
    broadcastAdminUpdate(); 
  });
  
  socket.on('typing', (data) => { 
    socket.to(data.roomId).emit('partner-typing', { isTyping: data.isTyping }); 
  });
  ...
  socket.on('leave-chat', (data) => {
    ...
    if (activeRooms.has(data.roomId)) { 
      const r = activeRooms.get(data.roomId); 
      const p = r.users.find(u => u.id !== socket.id); 
      if (p) { 
        io.to(p.id).emit('system-message', { text: 'Partner left.' }); 
        io.to(p.id).emit('partner-status', { online: false }); 
        const pu = allUsers.get(p.id); 
        if (pu) pu.room = null; 
      } 
      activeRooms.delete(data.roomId); 
    }
    const cu = allUsers.get(socket.id); 
    if (cu) cu.room = null; 
    broadcastAdminUpdate();
  });
  ...
  socket.on('disconnect', () => {
    ...
    const idx = waitingUsers.findIndex(w => w.id === socket.id); 
    if (idx !== -1) waitingUsers.splice(idx, 1);
    for (const [rid, r] of activeRooms.entries()) { 
      if (r.users.some(u => u.id === socket.id)) { 
        const p = r.users.find(u => u.id !== socket.id); 
        if (p) { 
          io.to(p.id).emit('partner-left'); 
          io.to(p.id).emit('system-message', { text: 'Partner disconnected.' }); 
          const pu = allUsers.get(p.id); 
          if (pu) pu.room = null; 
        } 
...
app.get('/api/health', (req, res) => {
  res.status(200).json({ 
    status: 'ok', 
    connections: allUsers.size,
    rooms: activeRooms.size,
    waiting: waitingUsers.length,
    posts: freedomWallPosts.length,
    stories: freedomWallStories.length,
...
app.post('/api/broadcast-request', (req, res) => {
  ...
  saveJSON(FILES.broadcastRequests, broadcastRequests); 
  io.emit('new-broadcast-request', request);
  console.log(`📨 New broadcast request from ${name}`); 
  res.json({ success: true, message: 'Broadcast request submitted!' });
});

app.get('/api/broadcast-requests', (req, res) => { 
  res.json(broadcastRequests); 
});
...
app.put('/api/broadcast-request/:id', (req, res) => {
  ...
  saveJSON(FILES.broadcastRequests, broadcastRequests); 
  io.emit('broadcast-request-updated', request); 
  res.json({ success: true, request });
});

// ============================================= FREEDOM WALL APIs =============================================
app.get('/api/freedom-wall/posts', (req, res) => { 
  res.json({ success: true, posts: freedomWallPosts }); 
});
...
app.post('/api/freedom-wall/posts', (req, res) => {
  ...
  freedomWallPosts.unshift(newPost); 
  if (freedomWallPosts.length > 500) freedomWallPosts = freedomWallPosts.slice(0, 500); 
  totalFWPosts++;
  saveJSON(FILES.freedomWallPosts, freedomWallPosts); 
  io.emit('freedom-wall-new-post', newPost);
  console.log(`📝 New Freedom Wall post from ${username}${images ? ` (${images.length} photos)` : ''}`); 
  broadcastAdminUpdate();
...
app.put('/api/freedom-wall/posts/:id/like', (req, res) => {
  ...
  saveJSON(FILES.freedomWallPosts, freedomWallPosts); 
  io.emit('freedom-wall-post-updated', { id, likes: post.likes, likedBy: post.likedBy });
...
app.delete('/api/freedom-wall/posts/:id', (req, res) => {
  ...
  freedomWallPosts.splice(postIndex, 1); 
  totalFWPosts = Math.max(0, totalFWPosts - 1);
  if (freedomWallComments[id]) { 
    totalFWComments -= freedomWallComments[id].length; 
    delete freedomWallComments[id]; 
  }
  saveJSON(FILES.freedomWallPosts, freedomWallPosts); 
  saveJSON(FILES.freedomWallComments, freedomWallComments);
  io.emit('freedom-wall-post-deleted', { id });
  console.log(`🗑️ Freedom Wall post deleted by ${username}`); 
  broadcastAdminUpdate();
...
app.get('/api/freedom-wall/comments/:postId', (req, res) => {
  ...
  const comments = freedomWallComments[postId] || [];
...
app.post('/api/freedom-wall/stories', (req, res) => {
  ...
  freedomWallStories.unshift(newStory); 
  if (freedomWallStories.length > 100) freedomWallStories = freedomWallStories.slice(0, 100); 
  totalFWStories++;
  saveJSON(FILES.freedomWallStories, freedomWallStories); 
  io.emit('freedom-wall-new-story', newStory);
  broadcastAdminUpdate(); 
...
app.delete('/api/freedom-wall/stories/:id', (req, res) => {
  ...
  freedomWallStories = freedomWallStories.filter(s => s.id !== id); 
  totalFWStories = Math.max(0, totalFWStories - 1);
  saveJSON(FILES.freedomWallStories, freedomWallStories); 
  io.emit('freedom-wall-story-deleted', { id });
...
app.post('/api/clear-data', (req, res) => {
  ...
  totalFWComments = 0; 
  totalUniqueUsers = 0;
  io.emit('announcement-dismiss', { id: 'clear-all' }); 
  io.emit('freedom-wall-cleared');
  totalMessages = 0; 
...
app.get('/api/export/users', (req, res) => {
  const users = []; 
  allUsers.forEach(v => users.push({ Name: v.name, IP: v.ip, SocketID: v.socketId, Status: v.status, ConnectedAt: v.connectedAt, Room: v.room || 'None' }));
  res.setHeader('Content-Type', 'text/csv'); 
  res.setHeader('Content-Disposition', 'attachment; filename=cremyxo-users.csv');
...
app.get('/api/export/messages', (req, res) => {
  res.setHeader('Content-Type', 'text/csv'); 
  res.setHeader('Content-Disposition', 'attachment; filename=cremyxo-messages.csv');
...
app.get('/api/export/freedom-wall', (req, res) => {
  res.setHeader('Content-Type', 'text/csv'); 
  res.setHeader('Content-Disposition', 'attachment; filename=cremyxo-freedom-wall.csv');
